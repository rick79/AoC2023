...
# --- Day 1: Trebuchet?! ---
#   
# omething is wrong with global snow production, and you've been selected to 
...
# Easy way: Split text, extract numbers using regex
list <- str_split_1(data, "\n") |> str_extract_all("\\d")
list
sum <- 0
for (l in list) {
  if(length(l) > 0) {
    sum <- sum + as.numeric(paste0(head(l, n=1), tail(l, n=1)))
  }
}
sum
# Answer: 55002

# Tibble way: Create a tibble with calibration values, group rowwise, mutate and
# concatenate numbers extracted using regex, ungroup and summarise.
tibble("calibration" = str_split_1(data, "\n"))  |> 
  rowwise() |> 
  mutate(
      extracted = str_extract_all(calibration, "\\d"),
      tmp = length(extracted),
      number = if_else(length(extracted) > 0, as.numeric(paste0(first(extracted),last(extracted))), 0) #,
  ) |> 
  ungroup() |> 
  summarise(
...
# Part Two ----------------------------------------------------------------
#
# Now we need to replace the strings one, two, ..., nine with the corresponding
# numbers. But we need to do so while taking into account that some numbers are 
# written overlapping, such as oneight. So instead of replacing the strings with
# the respecitve numbers we reaplace them with <string><number><string>. In the
# example above oneight becomes one1oneight8eight.
#


# Easy way: Do string replacement using regex, then split, extract numbers using 
# regex and concatenate to calculate the sum.
list2 <- text |> 
  str_replace_all("one", "one1one") |> 
  str_replace_all("two", "two2two") |> 
  str_replace_all("three", "three3three") |> 
  str_replace_all("four", "four4four") |> 
  str_replace_all("five", "five5five") |> 
  str_replace_all("six", "six6six") |> 
  str_replace_all("seven", "seven7seven") |> 
  str_replace_all("eight", "eight8eight") |> 
  str_replace_all("nine", "nine9nine") |> 
  str_split_1("\n") |> 
  str_extract_all("\\d")

sum2 <- 0
for (l in list2) {
  sum2 <- sum2 + as.numeric(head(l, n=1))*10 + as.numeric(tail(l, n=1))
}
sum2
# Answer: 55093


# Tibble way: Replace strings with regex, then proceed the same way as in the tibble way 
# of Part One.
tibble("calibration" = data |> 
  str_replace_all("one", "one1ene") |> 
  str_replace_all("two", "two2two") |> 
  str_replace_all("three", "three3three") |> 
  str_replace_all("four", "four4four") |> 
  str_replace_all("five", "five5five") |> 
  ...
  mutate(
    extracted = str_extract_all(calibration, "\\d"),
    number = as.numeric(paste0(first(extracted),last(extracted))),
    extracted = NULL
  ) |> 
  ungroup() |> 
  summarise(
...
tibble("calibration" = data |> 
         ...
         str_split_1("\n")) |> 
  mutate(
    number = unlist(lapply(str_extract_all(calibration, "\\d"), function(extracted) {return(as.numeric(paste0(first(extracted),last(extracted))))}))
  ) |> 
  summarise(
    sum = sum(number)
  )
