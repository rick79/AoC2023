...
# Part One ----------------------------------------------------------------
#
# Since I don't like to run for loops to check if things are equal up and 
# down matrices I hashed the rows and columns when checking symmetries. That 
# let me reduce it to a one-dimensinal problem. I had a plan to add a cache
# to the hashing function to speed things up but that was unnecessary.
#
#
patterns <- data |> 
  str_split_1("\\n\\n") |> 
...
# lines of reflection.
find_mirrors_hash <- function(m, dim = 1, exclude = -1) {
  v <- transmorph(m, dim)
  l <- length(v)
  r <- 0
  for(i in 1:(l-1)) {
...
# Part Two ----------------------------------------------------------------
#
# Time to unsmudge the mirrors. Rewriting the comparison function to take 
# distance between vectors into consideration sounded like to much work, much
# because I wouldn't be able to transmorph my matrices any longer.
#
# Easiest solution? Just create one matrix for each alteration to the original
# patterns and check each patterns new variations for a NEW line of reflection.
#
unsmudge <- function(m) {
  permutations <- list()
  permutations <- append(permutations, list(m))
  for(y in 1:nrow(m)) {
    for(x in 1:ncol(m)) {
      mp <- matrix(c(m), ncol=ncol(m), byrow = FALSE)
      if(mp[y, x] == "#") {
        mp[y, x] <- ".0"
      } else if (mp[y, x] == "."){
        mp[y, x] <- "#0"
      ...
      permutations <- append(permutations, list(mp))
    }
  }
  return(permutations)
}
...
lapply(fixes, \(x) {
  ...
  result <- 0
  for(i in 2:length(x)) {
    h <- find_mirrors_hash(x[[i]], 1, h0)
    v <- find_mirrors_hash(x[[i]], 2, v0)
    if(h + v > 0) {
      print(paste0("i = ", i, ", h = ", h, ", v = ", v))
      result <- result + v + h * 100
    }
  }
  result
}) |> unlist() |> sum()
