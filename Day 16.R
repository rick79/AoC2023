...
# Part One ----------------------------------------------------------------
#
# Again a quite simple problem. Tried solving it using recursion at first but
# ran into stack overflow quite quickly. Switched to using a que instead
#
layout <- data |> 
  str_split_1("\n") |> 
  str_split("") |> 
...
check_encounter <- function(e, d) {
  ...
  if(e == "\\") return(list(rev(d)))
  if(e == "|" & abs(d[2]) == 1) return(list(c(1, 0), c(-1, 0)))
  if(e == "-" & abs(d[1]) == 1) return(list(c(0, -1), c(0, 1)))
  return(list(d))  
}

traverse <- function(l, p, d, contraption, history, que) {
  if(history$has(list(p, d))) {
    return(contraption)
  }
  history$set(list(p, d), 1)
  repeat {
    p <- p + d
    if(p[1] < 1 | p[1] > nrow(l) | p[2] < 1 | p[2] > ncol(l)) {
      return(contraption)
    }
    contraption[p[1], p[2]] <- "#"
    if(l[p[1], p[2]] != ".") {
...
find_energized(layout, c(1, 0), c(0, 1))
# Answer: 7517
...
res_y <- lapply(1:nrow(layout), \(y) {
  ...
  max(find_energized(layout, c(y, 0), c(0, 1)), find_energized(layout, c(y, ncol(layout)), c(0, -1)))
})
res_x <- lapply(1:ncol(layout), \(x) {
  #print(paste0("Testing [1, ",x , "] and [", nrow(layout), ", ", x, "]"))
  max(find_energized(layout, c(0, x), c(1, 0)), find_energized(layout, c(nrow(layout), x), c(-1, 0)))
})
max(unlist(res_x), unlist(res_y))
