...
# Part One ----------------------------------------------------------------
#
# Lets begin by examine the data this time. The input consists of coordinates 
# separated by a ~. Each coordinate consists of a y, x and z component. There
# are two coordinates on each line denoting the start and end of a brick.
#
# My first assumption that I want to test is whether the bricks can span in more
# than one direction. This is true for both the test data and real data. A brick
# either consists of a single cube or a single line of cubes. So no tetris 
# shapes or anything like that.
#
# Brick from [z = 2, y = 2, x = 2] to [z = 2, y = 2, x = 5]
# 
#   |             |             |
# y |  ####     y |  #        z |  ###
...
#       x             z              x
#
# So basically we're meant to play tetris with simplified pieces but in 3D. I'm
# sure there is a clever way to do this, but I think I'll just enjoy reminiscing 
# simpler days where processor speeds was measured in MHz and not Ghz and 
# absolutely nobody ever needed more than 68k of memory. Never and forever.
#
# Tetris is usually implemented using a simple boolean grid but since we'll need
# to access specific pieces I'll use a 3D integer array.
#
# This is a fairly naive way of solving the problem, but it works and was fun
# to write.
#

bricks <- data |> str_split_1("\\n") |> 
  str_split("~") |> 
  lapply(\(x) {
    d1 <- x[1] |> str_split_1(",") |> as.integer()
    d2 <- x[2] |> str_split_1(",") |> as.integer()
    list(start = list(y = d1[2] + 1, x = d1[1] + 1, z = d1[3] + 1), end = list(y = d2[2] + 1, x = d2[1] + 1, z = d2[3] + 1), id = 0 )
  })
...
# Are all bricks either 1x1x1 cubes or 1x1xN shapes?
stopifnot(all((bricks |> lapply(\(x) veq(x$start, x$end)) |> unlist()) %in% c(2, 3)))

# Find the max an min values of the pieces Probably won't need the min values.
mm <- bricks |> unlist() |> {\(l) { list(
  max.x = max(l[names(l) == "start.x" | names(l) == "end.x"]),
  min.x = min(l[names(l) == "start.x" | names(l) == "end.x"]),
  max.y = max(l[names(l) == "start.y" | names(l) == "end.y"]),
  min.y = min(l[names(l) == "start.y" | names(l) == "end.y"]),
  max.z = max(l[names(l) == "start.z" | names(l) == "end.z"]),
  min.z = min(l[names(l) == "start.z" | names(l) == "end.z"])
)}}()

# No bricks have a position with a coordinate that is less than 0
stopifnot(min(mm$min.x, mm$min.y, mm$min.z) >= 0)

space <- array(0L, dim = c(mm$max.z, mm$max.y, mm$max.x))
...
getids <- function(space, z) {
  ids <- space[z,,] |> as.vector() |> unique()
  ids <- ids[ids != 0]
  return(ids)
}

# Move all pieces that are at a specific z value. This means that pieces that
# stretch over several z values will be broken up. But it doesn't matter since
# they'll be reunited when the layer above moves.
# The ignore parameter is to handle checking what happens if a piece is removed
# by disintegration.
move <- function(space, z, ignore = NULL) {
  ids <- getids(space, z)
  ignore_list <- c(0, ignore)
  moved_bricks <- c()
  for(id in ids) {
    if(id %in% ignore_list) next
    pos <- which(space[z,,] == id, arr.ind = TRUE)
    zlist <- c()
    for(i in 1:nrow(pos)) {
      high.z <- z
      if(z > 1) {
        for(dz in (z-1):1) {
          if(space[dz, pos[i,1], pos[i, 2]] %in% ignore_list) high.z <- dz
          else break
        }
      }
      zlist <- append(zlist, high.z)
    }
    new.z <- max(zlist)
    if(new.z < z) {
      for(i in 1:nrow(pos)) {
        space[z, pos[i, 1], pos[i, 2]] <- 0
        space[new.z, pos[i, 1], pos[i, 2]] <- id
      }
      if(!id %in% moved_bricks) moved_bricks <- append(moved_bricks, id)
    }
  }
  return(list(space = space, moves = moved_bricks |> unique() |> length(), moved_bricks = moved_bricks |> unique()))
}

disintegrate <- function(space) {
  seen <- list()
  counter <- 0
  for(z in (dim(space)[1]-1):1) {
    ids <- getids(space, z)
    if(length(ids) > 0) {
      for(id in ids) {
        if(id %in% seen) next
        seen <- append(seen, id)
        t <- move(space, z+1, ignore = id)
        if(t$moves == 0) counter <- counter + 1
      }
    }
  }
  return(counter)
...
# Part Two ----------------------------------------------------------------
#
# Probably one of the fastest Part Twos. Not much that had to be changed from 
# part one to be honest.
#

# What's better than disintegrating stuff? Exterminate! Exterminate! Exterminate!
exterminate <- function(space) {
  seen <- list()
  counter <- 0
  for(z in (dim(space)[1]-1):1) {
    ids <- getids(space, z)
    if(length(ids) > 0) {
      for(id in ids) {
        ...
        moved_bricks <- c()
        tspc <- space
        for(i in z:dim(tspc)[1]) {
          res <- move(tspc, i, ignore = id)
          moved_bricks <- append(moved_bricks, res$moved_bricks)
          tspc <- res$space
        }
        counter <- counter + moved_bricks |> unique() |> length()
        # print(paste0(counter, " bricks would fall if brick ", id, " was disintegrated"))
      }
    }
  }
  return(counter)
