...
data <- "btl{m>2179:hzd,a>965:js,x<3167:bc,crx}
...
# Part One ----------------------------------------------------------------
#
# Part One was pretty straight forward. The hardest part was to split the input 
# data into a usable format. I tried different ways with matrices and lists but
# settled on using lists. The only downside is the clunky filtering for nestled
# named lists.
#
...
workflows <- input |> 
  ...
  lapply(\(x) {
    list(
      name = str_extract(x, "^[a-z]+(?=\\{)"),
      catchall = str_extract(x, "[A|R|a-z]+(?=\\})"),
      rules = lapply(str_split_1(str_extract(x, "(?<=\\{).*(?=\\,[A|R|a-z]+\\})"), ","), \(r) {
        list(
          property = str_extract(r, "^[x|m|a|s]"),
          conditional = str_extract(r, "[\\<|\\>]"),
          target = strtoi(str_extract(r, "\\d+")),
          action = str_extract(r, "[A|R|a-z]+$")
        )
      }) 
    )
  })
...
parts <- input |> 
  ...
  str_split_1("\n") |> 
  lapply(\(x) list( 
                x = strtoi(str_extract(x, "(?<=x\\=)\\d+")), 
                m = strtoi(str_extract(x, "(?<=m\\=)\\d+")), 
                a = strtoi(str_extract(x, "(?<=a\\=)\\d+")), 
                s = strtoi(str_extract(x, "(?<=s\\=)\\d+"))
  ))
...
process_parts <- function(part, wf = "in") {
  ...
  for(rule in rules) {
    ...
    if(cond) {
      ...
      if(rule$action == "R") return(FALSE)
      return(process_parts(part, rule$action))
    }
  }
  if(workflow$catchall == "A") return(TRUE)
  if(workflow$catchall == "R") return(FALSE)
  return(process_parts(part, workflow$catchall))
}
...
# Part Two ----------------------------------------------------------------
#
# Flashback to Day 5(?) This part took a while to get right. I'm happy that 
# recursion worked for this one. Had some problems with the check if there
# is anything left in a range resulting in huge nestled if-else-whatever 
# constructs. I looked at other solutions but everyone had done something little
# different so that didn't help. In the end I found someone who had done 
# something somewhat like me and could take some inspiration from there. The 
# exhausted flag also comes from there. Stole that with pride as usual.
#
# And I think that R's pass by value design is giving me a bad habit of 
# referencing data structures from outside in my functions.
#
find_combinations <- function(ranges, node = "in") {
  ...
  for(rule in workflow$rules) {
    range <- ranges[[rule$property]]
    if(rule$conditional == "<") {
      ar <- c(range[1], rule$target - 1)
      rr <- c(rule$target, range[2])
    } else {
      ar <- c(rule$target + 1, range[2])
      rr <- c(range[1], rule$target)
    }
    if(ar[1] <= ar[2]) { # Is the lower bound less than or equal to the upper bound? If so there is something left in this range!
      nr <- ranges
      nr[[rule$property]] <- ar
      combinations <- combinations + find_combinations(nr, rule$action)
    }
    if(rr[1] <= rr[2]) { # Is the lower bound of the rest of the range less than or equal to the upper bound? If so we have something to pass to the next rule in the chain!
      ranges[[rule$property]] <- rr
    } else {
      exhausted <- TRUE # There is nothing left to pass to the next rule in the chain!
      break
    }
  }
  if(!exhausted) { # If there are something left to pass to the catchall send it that way!
    combinations <- combinations + find_combinations(ranges, workflow$catchall)
  }
  return(combinations)
}
...
print(find_combinations(ranges), digits = 20)
