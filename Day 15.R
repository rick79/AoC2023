...
data <- paste(readLines("day15.txt"), collapse="")
...
holiday_ash <- function(string) {
  current_value <- 0
  for(c in utf8ToInt(string)) {
...
for(ins in instructions) {
  ...
  lenses <- boxes$get(ins$box, ordered_dict())
  if(ins$op == "=") lenses$set(ins$label, ins$focal_length) else lenses$remove(ins$label, silent = TRUE)
  boxes$set(ins$box, lenses)
}

fp <- 0
for(key in boxes$keys() |> unlist() |> sort()) {
  lenses <- boxes$get(key)
  labels <- lenses$keys()
  if(length(labels) > 0) {
    for(i in 1:length(labels)) {
      fp <- fp + (key + 1) * i * lenses$get(labels[[i]])
    }
  }
}
print(fp)
...
for(key in boxes$keys() |> unlist() |> sort()) {
  lenses <- boxes$get(key)
  if(length(lenses) > 0) {
    for(i in 1:length(lenses)) {
      fp <- fp + (key + 1) * i * lenses[[i]]
    }
  }
}
print(fp)
