...
# Camel Cards. Camel Cards is sort of similar to poker except it's designed to be easier to play while riding a camel.
#
...
# Part One ----------------------------------------------------------------
# Hand              Example Card counts    Strength
# Five of a kind    AAAAA   5              7
# Four of a kind    AA8AA   4, 1           6
# Full house        23332   3, 2           5
# Three of a kind   TTT98   3, 1, 1        4
# Two pair          23432   2, 2, 1        3
...
strength <- list(
  c(1L, 1L, 1L, 1L, 1L),  # High card
  c(1L, 1L, 1L, 2L),      # One pair
  c(1L, 2L, 2L),          # Two pairs
  c(1L, 1L, 3L),          # Three of a kind
  c(2L, 3L),              # Full house
  c(1L, 4L),              # Four of a kind
  c(5L)                   # Five of a kind
)
...
get_hand_strength <- function(cards) {
  ...
  s <- case_when(
    ...
    identical(cs, strength[[4]]) ~ 4, # Three of a kind
    identical(cs, strength[[5]]) ~ 5, # Full house
    identical(cs, strength[[6]]) ~ 6, # Four of a kind
    identical(cs, strength[[7]]) ~ 7, # Five of a kind
    .default = 1                      # High card
  )
  return(s)
}
...
get_hand_strength_wildcard <- function(hand) {
  ...
  s <- case_when(
    identical(cs, strength[[2]]) ~ 2,
    identical(cs, strength[[3]]) ~ 3,
    identical(cs, strength[[4]]) ~ 4,
    identical(cs, strength[[5]]) ~ 5,
    identical(cs, strength[[6]]) ~ 6,
    identical(cs, strength[[7]]) ~ 7,
    .default = 1
  )
  if(!is.na(wc)) {
    s <- case_when(
      (s == 1) ~ 2,             # High card (1)          => One pair (2)
      (s == 2) ~ 4,             # One pair (2)           => Three of a kind (4)
      (s == 3) & (wc == 1) ~ 5, # Two pairs 1J (3)       => Full house (5)
      (s == 3) & (wc == 2) ~ 6, # Two pairs 2J (3)       => Four of a kind (6)
      (s == 4) ~ 6,             # Three of a kind (4)    => Four of a kind (6)
      (s == 5) ~ 7,             # Full house (5)         => Five of a kind (7)
      (s == 6) ~ 7,             # Four of a kind (6)     => Five of a kind (7)
      .default = s                    
    )
  }
  return(s)
